...
'''
Introduce and tune L2 regularization for both logistic and neural network models. Remember that L2 amounts to adding a
 penalty on the norm of the weights to the loss. In TensorFlow, you can compute the L2 loss for a tensor t using
  nn.l2_loss(t). The right amount of regularization should improve your validation / test accuracy.
'''

'''
# Normal logistics Model
print("Normal Logistics Model")
train_subset = 10000
graph = tf.Graph()
...
with graph.as_default():
    ...
    layer2 = tf.nn.softmax(logits_one)   # its hidden laye one
    logits_two = tf.matmul(layer2,weights_three)+biases_three
    layer1 = tf.nn.dropout(tf.nn.relu(logits_two) , keep_prob )
    # h1 = tf.nn.dropout( tf.sigmoid( tf.matmul( x0,m1 ) + b1 ) , keep_prob )
    logits = tf.matmul(layer1,weights_two)+biases_two
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels,logits=logits))
    loss = tf.reduce_mean(loss+0.01*tf.nn.l2_loss(weights_one))
...
with tf.Session(graph=graph) as session:
  ...
  for step in range(num_steps):
    ...
    feed_dict = {tf_train_dataset : batch_data, tf_train_labels : batch_labels,keep_prob:0.5,learnr:0.0002}
...
with graph.as_default():
    ...
    logits = tf.matmul(layer1,weights_two)+biases_two
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels,logits=logits))
    loss = tf.reduce_mean(loss+0.01*tf.nn.l2_loss(weights_one)+0.01*tf.nn.l2_loss(weights_two)+0.01*tf.nn.l2_loss(weights_three))
    optimizer = tf.train.GradientDescentOptimizer(0.5).minimize(loss)
    train_prediction = tf.nn.softmax(logits)
    test_prediction = tf.nn.softmax(tf.matmul(tf.nn.relu(tf.matmul(tf.nn.softmax(tf.matmul(tf_test_dataset, weights_one)+biases_one),weights_three)+biases_three),weights_two)+biases_two)
    valid_prediction =tf.nn.softmax(tf.matmul(tf.nn.relu(tf.matmul(tf.nn.softmax(tf.matmul(tf_valid_dataset,weights_one)+biases_one),weights_three)+biases_three),weights_two)+biases_two)
